...
function SignupInner() {
  ...
  function handleGoogleSignup() {
    ...
    /*
     * These values can be read by your Google callback page.
     * The intent value identifies that Google authentication
     * began from the signup page.
     */
    sessionStorage.setItem("pwa_google_intent", "signup");
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      /*
       * saveSession keeps the token and user information.
       * This matches the login page's authentication flow.
       */
    ...
    <main className="flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-gold/20 bg-white/60 p-8 shadow-xl backdrop-blur dark:bg-deep/60">
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-3 rounded-lg border border-black/15 bg-white px-4 py-2.5 font-medium text-black transition hover:bg-black/5 disabled:cursor-not-allowed disabled:opacity-50"
        >
          <svg
            aria-hidden="true"
            viewBox="0 0 24 24"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              autoComplete="name"
              disabled={formDisabled}
              className="mt-1 w-full rounded-lg border border-gold/30 bg-transparent px-3 py-2 focus:outline-none focus:ring-2 focus:ring-gold disabled:cursor-not-allowed disabled:opacity-60"
              value={form.name}
              onChange={(event) =>
                setForm((currentForm) => ({
                  ...currentForm,
