...
function LessonPlayerInner() {
  ...
  const [activeLessonId, setActiveLessonId] = useState(searchParams.get("lesson"));
  ...
  useEffect(() => {
    ...
    courseApi
      ...
      .then((data) => {
        ...
        if (!data.enrolled) return; // not enrolled — nothing more to prep

        const flat = flattenLessons(data.course);
        const requested = searchParams.get("lesson");
        const initial =
          flat.find((l) => l.id === requested) ||
  ...
  async function handleMarkComplete() {
    ...
    try {
      ...
      // Refresh course data so sidebar checkmarks and progress bar update
  ...
  if (loading) return <main className="p-8">Loading lesson...</main>;
  if (error) return <main className="p-8 text-red-600">{error}</main>;
  if (enrolled === false) {
    return (
      <main className="max-w-lg mx-auto px-4 py-24 text-center">
        <h1 className="font-display text-2xl mb-3">You're not enrolled in this course yet</h1>
        <p className="opacity-70 mb-6">Enroll to unlock the full lesson player and track your progress.</p>
        <a href={`/courses/${slug}`} className="px-6 py-2.5 rounded-lg bg-gold text-ink font-semibold">
          View course
        </a>
      </main>
    );
    ...
    <div className="flex flex-col md:flex-row min-h-screen">
      ...
      <aside className="md:w-80 border-b md:border-b-0 md:border-r border-gold/20 bg-white/40 dark:bg-deep/40">
        <div className="p-4 border-b border-gold/10">
          <a href={`/courses/${slug}`} className="text-sm opacity-60 hover:opacity-100">
            ← Back to course
          </a>
        ...
        <div className="max-h-[70vh] md:max-h-[calc(100vh-140px)] overflow-y-auto">
          {course.sections.map((section) => (
            <div key={section.id} className="border-b border-gold/10">
              <button
                onClick={() => toggleSection(section.id)}
                className="w-full flex justify-between items-center px-4 py-3 text-left font-medium text-sm"
              >
                {section.title}
                <span className="opacity-50">{collapsedSections[section.id] ? "+" : "−"}</span>
              </button>
                    ...
                    <li key={lesson.id}>
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 hover:bg-gold/10 transition ${
                          lesson.id === activeLessonId ? "bg-gold/15 font-medium" : ""
                        }`}
                      >
        ...
        {activeLesson ? (
          <>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mt-6">
              ...
              >
                {activeLesson.completed ? "Completed ✅" : marking ? "Saving..." : "Mark as complete"}
              </button>
            </div>
